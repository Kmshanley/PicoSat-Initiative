...
def total_area_sum_from_ohms() -> float:
    '''
    Given the ohms per interior layer, calculates the area-sum
    of the magnetorquer.

    Parameters:
        ext_ohms (float): the resistance (in ohms) per interior layer
    Returns:
        - The total area_sum given this constraint
    ...
    int_ohms = config.getint("Resistance")
    int_layers = config.getint("NumberOfLayers")
    area_sum = int_layers * spiral_of_resistance(int_ohms, False)[0]
    return area_sum
...
if __name__ == "__main__":
    ...
    int_ohms = config.getfloat("Resistance")
    interior = spiral_of_resistance(int_ohms / config.getint("NumberOfLayers"), False)

    # Print information about optimal magnetorquer
    total_area_sum = total_area_sum_from_ohms()
    print("Optimal properties calculated given config.ini:")
    print(f"Total area-sum: {total_area_sum:.4f} m^2\n")
    interior_layers = config.getint("NumberOfLayers")
    ...
    output_KiCad_square_spiral.save_magnetorquer(interior[3], interior[2])
