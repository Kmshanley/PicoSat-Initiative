 ...
 */
W25N_STATE W25N_ReadJEDECID(W25Nxx_dev * dev, uint64_t * buf) {
 ...
 * @return W25N_STATE enum
 */
W25N_STATE W25N_Read_Status_Reg(W25Nxx_dev * dev, uint8_t SRAddress, uint16_t * reg)
{
	OSPI_RegularCmdTypeDef com = {0};
...
W25N_STATE W25N_Wait_Until_Not_Busy(W25Nxx_dev * dev, uint32_t timout_ms)
{
	...
	com.AlternateBytesMode = HAL_OSPI_ALTERNATE_BYTES_1_LINE;
	com.AlternateBytesSize = HAL_OSPI_ALTERNATE_BYTES_8_BITS;
	com.AlternateBytes = W25N_STATUS_REG;
	...
	polling_cmd.Mask = 0b00000001; //Busy bit is LSB, the only one we care about
	polling_cmd.Match = 0b00000000; //Busy bit is 0 when not busy
